 ...
 *   iconType - 控制图标外观：'folder' | 'module' | 'item' | 自定义字符串
...
export interface TreeNode {
  ...
  code: string
  version: string
  iconType?: 'folder' | 'module' | 'item' | string
  hasChild?: boolean
  [key: string]: unknown
}
...
/* ════════════════════════════════════════
   ...
   - 通过 BEM modifier 切换颜色：
       icon-folder  → 橙色（#f59e0b）
       icon-module  → 蓝色（#3b82f6）
       icon-item    → 灰色（#6b7280）
